...
@testset "MP: Lₙ⁽ᵅ⁾(x) = x⁻ᵅeˣ/n! dⁿ/dxⁿ xⁿ⁺ᵅe⁻ˣ" begin
  ...
  for α in 0:n
    ...
    f = simplify(Antique.laguerre(MP, x, n=n, α=α), expand=true) # closed-form
    # latexify
...
@testset "MP: ∫Lᵢ⁽ᵅ⁾(x)Lⱼ⁽ᵅ⁾(x)xᵅexp(-x)dx = Γ(i+α+1)/i! δᵢⱼ" begin
  ...
  for j in 0:9
    ...
    numerical  = quadgk(x -> real(Antique.laguerre(MP, x, n=i, α=α)) * real(Antique.laguerre(MP, x, n=j, α=α)) * x^α * exp(-x), 0, Inf, maxevals=10^3)[1]
