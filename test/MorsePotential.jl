...
@testset "MP: Lₙ⁽ᵅ⁾(x) = x⁻ᵅeˣ/n! dⁿ/dxⁿ xⁿ⁺ᵅe⁻ˣ" begin
    ...
    for n in 0:4
        for α in 0:n
            ...
            f = simplify(Antique.laguerre_polynomial(MP, x, n = n, alpha = α), expand = true) # closed-form
            # latexify
    ...
    for α in [0.01, 0.1, 1.0]
        for i in 0:9
            for j in 0:9
                ...
                numerical = quadgk(x -> real(Antique.laguerre_polynomial(MP, x, n = i, alpha = α)) * real(Antique.laguerre_polynomial(MP, x, n = j, alpha = α)) * x^α * exp(-x), 0, Inf, maxevals = 10^3)[1]
