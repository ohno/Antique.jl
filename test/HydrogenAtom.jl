...
@testset "HA: Pₙᵐ(x) = √(1-x²)ᵐ dᵐ/dxᵐ Pₙ(x); Pₙ(x) = 1/(2ⁿn!) dⁿ/dxⁿ (x²-1)ⁿ" begin
  ...
  for m in 0:n
      ...
      f = simplify(Antique.rodrigues_formula(HA, x, n=n, m=m), expand=true) # closed-form
      # latexify
...
@testset "HA: ∫Pᵢᵐ(x)Pⱼᵐ(x)dx = 2(j+m)!/(2j+1)(j-m)! δᵢⱼ" begin
  ...
  for j in m:9
    ...
    numerical  = quadgk(x -> Antique.rodrigues_formula(HA, x, n=i, m=m) * Antique.rodrigues_formula(HA, x, n=j, m=m), -1, 1, maxevals=10^3)[1]
...
@testset "HA: Lₙᵏ(x) = dᵏ/dxᵏ Lₙ(x); Lₙ(x) = 1/(n!) eˣ dⁿ/dxⁿ e⁻ˣ xⁿ" begin
  ...
  for k in 0:n
    ...
    d = expand_derivatives(c)                                    # expand dⁿ/dxⁿ and dᵐ/dxᵐ
    e = simplify(d, expand=true)                                 # simplify
    f = simplify(Antique.laguerre(HA, x, n=n, k=k), expand=true)        # closed-form
...
@testset "HA: ∫exp(-x)xᵏLᵢᵏ(x)Lⱼᵏ(x)dx = (2i+k)!/(i+k)! δᵢⱼ" begin
  ...
  for k in 0:min(i,j)
    ...
    numerical  = quadgk(x -> exp(-x) * x^k * Antique.laguerre(HA, x, n=i, k=k) * Antique.laguerre(HA, x, n=j, k=k), 0, Inf, maxevals=10^3)[1]
    acceptance = iszero(analytical) ? isapprox(analytical, numerical, atol=1e-5) : isapprox(analytical, numerical, rtol=1e-5)
    @test acceptance
