...
#### Potential
```@docs; canonical=false
Antique.potential(::HydrogenAtom, ::Any)
```

#### Eigenvalues
```@docs; canonical=false
Antique.energy(::HydrogenAtom)
```

#### Eigenfunctions
```@docs; canonical=false
Antique.wavefunction(::HydrogenAtom, ::Any, ::Any, ::Any)
```

#### Radial Functions
```@docs; canonical=false
Antique.radial_function(::HydrogenAtom, ::Any)
```
...
#### Spherical Harmonics
```@docs; canonical=false
Antique.spherical_harmonic(::HydrogenAtom, ::Any, ::Any)
```
...
println("ΔE = ", energy(H,n=2) - energy(H,n=1), " Eₕ")
println("λ  = ", ((energy(H,n=2)-energy(H,n=1))*Eₕ2nm⁻¹)^-1, " nm")
```
...
lines!(ax, 0.1:0.01:20, r -> potential(H, r))
f
```
...
```@example HA
import Antique
H = Antique.HydrogenAtom(Z=1, m_e=1.0, a_0=1.0, E_h=1.0, hbar=1.0)
@show Antique.energy(H)
...
lines!(ax, 0..5, r -> abs(Antique.wavefunction(H,r,0,0)), linestyle=:dash, color=:black, label="Antique.jl")
...
for n in 1:3
  for l in 0:n-1
    w = lines!(
        ax,
        0..20,
        r -> r^2 * Antique.radial_function(H,r,n=n,l=l)^2,
        linewidth = 2,
...
using Markdown
using Antique
Markdown.parse(Base.read("../../test/result/HydrogenAtom.md", String))
```
