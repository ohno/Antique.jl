...
#### Hermite Polynomials
```@docs; canonical=false
Antique.hermite_polynomial(::HarmonicOscillator, ::Any)
```

## Usage & Examples

[Install Antique.jl](@ref Install) for the first use and run `using Antique` before each use. The energy `energy()`, wave function `wavefunction()` and potential `potential()` will be exported. In this system, the model is generated by `HarmonicOscillator` and several parameters `k`, `m` and `hbar` are set as optional arguments.
...
using Markdown
using Antique
Markdown.parse(Base.read("../../test/result/HarmonicOscillator.md", String))
```
